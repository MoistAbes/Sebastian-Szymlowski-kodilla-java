...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        long allTasksInProgress = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .count();
        ...
        long sumOfTheDaysAfterOrderingTasks = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        double averageDaysWorkingOnTask = (double) sumOfTheDaysAfterOrderingTasks / allTasksInProgress;


        //Then
        assertEquals(3, allTasksInProgress);
        assertEquals(30, sumOfTheDaysAfterOrderingTasks);
        //tylko to rrzeba do zadania
